  ...
  useEffect(() => {
    ...
    const createCluster = (centerX, centerY, count, spread, sizeRange) => {
      for (let i = 0; i < count; i++) {
        ...
        const x = Math.max(
          5,
          Math.min(95, centerX + Math.cos(angle) * distance)
        );
        const y = Math.max(
          5,
          Math.min(95, centerY + Math.sin(angle) * distance)
        );
  ...
  useEffect(() => {
    const animateParticles = () => {
      ...
      particlesRef.current = particlesRef.current.map((particle) => {
        ...
        const newX =
          particle.originalX +
          Math.sin(time * 0.2 + particle.phase) * floatRange;
        const newY =
          particle.originalY +
          Math.cos(time * 0.15 + particle.phase) * floatRange;
        ...
        const colorIndex =
          Math.floor((time + particle.id) * 0.1) % colors.length;
        let flowColor =
          particle.clusterType === "main" ? colors[colorIndex] : "#64b5f6";
          ...
          if (distanceFromCenter <= currentWave) {
            ...
            if (timeSinceReached >= 0) {
              ...
              if (timeSinceReached <= fadeInDuration) {
                ...
                heartbeatBoost =
                  Math.sin(fadeInProgress * Math.PI * 0.5) *
                  heartbeat.intensity;
                ...
                const fadeOutProgress =
                  (timeSinceReached - fadeOutStart) / fadeOutDuration;
                const fadeOutFactor = Math.cos(fadeOutProgress * Math.PI * 0.5);
                heartbeatBoost = Math.max(
                  0,
                  fadeOutFactor * heartbeat.intensity
                );
              }
              ...
              const organicVariation =
                0.1 + 0.1 * Math.sin(time * 2 + particle.id * 0.1);
        ...
        if (particle.heartbeatHistory.length > 10)
          particle.heartbeatHistory.shift();

        // Calculate trailing effect
        const averageBoost =
          particle.heartbeatHistory.reduce((a, b) => a + b, 0) /
          particle.heartbeatHistory.length;
        const smoothBoost = heartbeatBoost * 0.7 + averageBoost * 0.3;

        // Base opacity with smooth heartbeat boost
        const blinkFactor =
          0.4 +
          0.6 * Math.abs(Math.sin(time * particle.blinkSpeed + particle.phase));
        const baseOpacity =
          particle.baseOpacity *
          blinkFactor *
          (0.3 + 0.7 * Math.sin(time * 0.5));
        const finalOpacity = Math.max(
          baseOpacity,
          baseOpacity + smoothBoost * 0.8
        );
    ...
    <div className="opx-logo-ring-container">
      ...
      <div className="background-video">
        <video autoPlay loop muted playsInline className="bg-video">
          <source
            src="https://videos.pexels.com/video-files/8533458/8533458-uhd_2560_1440_24fps.mp4"
            type="video/mp4"
          />
        </video>
